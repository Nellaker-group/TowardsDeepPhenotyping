...
def read_image_bgr_val(path):
    image = np.asarray(PIL.Image.open(path).convert('RGB'))
    image_trans = image.copy()
#    if random.uniform(0,1) >= 0.3:
#      image_trans = rgb2hed(image_trans)
#      image_trans *= random.uniform(0.95,1.05)
#      image_trans = hed2rgb(image_trans)
#      image_trans = img_as_ubyte(image_trans)
    return image_trans[:, :, ::-1].copy()
...
#      image_trans = hed2rgb(image_trans)
#      image_trans = img_as_ubyte(image_trans)
      #print("HED augmentation happened")
    return image_trans[:, :, ::-1].copy()
